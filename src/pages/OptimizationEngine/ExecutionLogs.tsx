...
const ExecutionLogsPresenter = ({ logs }: ExecutionLogsProps) => {
  return (
    <div className="bg-white dark:bg-[#1a1a2e] rounded-2xl shadow-xl p-6 border border-[#015EB0]/10 dark:border-[#015EB0]/20 animate-slide-in">
      ...
      <div className="bg-[#002E5E] dark:bg-[#0a0a1a] rounded-xl p-4 h-64 overflow-y-auto border-2 border-[#015EB0]/20 dark:border-[#015EB0]/30">
        {logs.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-12 h-12 mx-auto mb-4 bg-[#015EB0]/20 dark:bg-[#3EA32A]/20 rounded-full flex items-center justify-center">
              <div className="w-6 h-6 bg-[#015EB0]/30 dark:bg-[#3EA32A]/30 rounded-full"></div>
            </div>
            <p className="text-gray-400 dark:text-gray-500 text-sm font-semibold">Esperando ejecución...</p>
          </div>
        ) : (
          <pre className="text-[#3EA32A] dark:text-[#3EA32A] text-sm font-mono">
            {logs.map((log, index) => (
